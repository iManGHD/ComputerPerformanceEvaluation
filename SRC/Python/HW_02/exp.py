...
if __name__ == "__main__":
    ...
    for lamba in np.arange(0.1,20.1,0.1):
        ...
        Nc_experiment = Nc_total / c

        print("lambda : ", lamba)
        print(pb_experiment, "\t", pb_analytics, "\t\t", pd_experiment, "\t", pd_analytics,  "\t\t", Nc_experiment, "\t", Nc_analytics,"\n")
        with open('exp.txt', 'a') as f:
            f.writelines(str(pb_experiment) + "\t" + str(pb_analytics) + "\t\t" + str(pd_experiment) + "\t" + str(pd_analytics) + "\t\t" + str(Nc_experiment) + "\t" + str(Nc_analytics) + "\n")
